#!/usr/bin/env python3
"""Fetch a full Langfuse trace replay as local JSON (log-triage skill bundled copy).
...
This is a bundled copy of ``scripts/fetch_langfuse_replay.py``; only ``repository_root`` is
modified to locate the real repo root from any invocation directory (walks upward to find the
directory containing ``apps/backend``). All other logic mirrors the upstream script.
"""
...
def repository_root() -> Path:
    """Return the repository root inferred by walking upward to find ``apps/backend``.
