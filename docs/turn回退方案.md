...
> **方案演进说明（2026-08-04）**：本文档原定的「整 turn 原地回退」已收敛为
> **task 级累积变更集 + 单文件保留/撤销**（`change_set_service` + 右侧「变更」面板）。
> 原方案的「清 checkpoint / turn_messages 整 turn 抹掉」因会造成上下文悬空、同文件
> 后续改动被覆盖等撕裂态，改为不触碰对话轨迹、仅对 `file_snapshots` 中已稳定的
> 单文件变更做反向 V4A 还原。具体实施以实现计划
> `docs/superpowers/plans/2026-08-04-task-change-set.md` 为准，本文档保留作历史
> 设计说明。
