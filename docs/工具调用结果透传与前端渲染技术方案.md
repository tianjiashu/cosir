...
> ⚠️ **本方案已被推翻（历史文档）**：后续决议改为「后端彻底不承载任何渲染逻辑，渲染全权交客户端」。
> 当前落地架构：`ToolDisplayHints` 退化为纯静态声明（`verb`/`icon`/`expandable`/`expand_layout`），
> 删除所有 `render_request_summary` / `render_result_summary` 方法与 `result_summary_template` / `render_fn`；
> 折叠态摘要、结果投影、list/diff 条目改由客户端共享渲染层
> `apps/shared/ts/toolDisplayRules.ts` 按工具名规则表生成；展示数据经 `tool_call_finished.data`
> 透传，由 `DisplayDataBudget` 守卫做展示通道输出治理。下文保留当初的方案推导供追溯。

> 原方案摘要（已废弃）：把 `tool_call_finished` 扩展为携带结果摘要/正文/失败原因的载体，并给
> `ToolDisplayHints` 新增 `result_summary_template` + `render_fn` 逃生舱，由后端声明式渲染结果摘要。
> 该「后端渲染」思路已被「客户端渲染」决议取代。
