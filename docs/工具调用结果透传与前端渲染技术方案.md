...
| patch_tool | `已修改 {file_count} 个文件 · {change_count} 处改动` | `file_count`, `change_count` |
| search_files | `命中 {match_count} 处` | `match_count` |
| list_directory | `{total_count} 个条目` | `total_count` |
| delete | `已删除 {type} · {count} 个` | `type`, `count` |
...
- `try: self.result_summary_template.format(**data) except (KeyError, ValueError): None`（与 `_render_summary` 同款容错）
...
if display is not None and display.result_summary_template is not None:
    result_summary = display.render_result(observation.data)
...
- `data` 已有 `path`，补 `"match_count": len(results)`（`results` 即搜索结果列表）

**`list_directory.py`**（`tool_success` 构造点）：
- `data` 已有 `details`，补 `"total_count": len(entries)`、`"path_basename": os.path.basename(path)`

**`delete.py`**（`tool_success` 构造点）：
- `data` 已有 `targets`：补 `"count": len(results)`、`"type": "directory" if is_dir else "file"`、`"path_basename": os.path.basename(str(targets[0]))`

**`read_file.py`**（`tool_success` 构造点）：
- `data` 已有 `line_count` 和 `path_basename` → 无需补

**`patch_tool.py`**（`tool_success` 构造点）：
- `data` 已有 `change_count`、`mode`、`"file_count"` → `change_count` 已有（`diff_stats.modified`），补 `"file_count": len(results)`
