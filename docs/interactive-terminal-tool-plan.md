...
- Assistant Transport 传输对话、Run 和工具状态，不承载完整 PTY 原始字节流；
...
`terminal_write` 应增加进程内的 `operation_id`/`command_id` 去重语义：同一个 session 内同一个 operation id 只允许写入一次；去重状态只保存在当前 session actor 内，不新增数据库表；backend 重启后 session 已经 interrupted，不自动重试旧写入；前端没有写入权限，因此不需要为 UI 设计写入幂等协议。

输出读取使用单调递增的 `output_seq`：`after_seq` 表示调用方已经完整应用的最后一个序号；返回 `seq > after_seq` 的增量和 `next_seq`；ring buffer 无法覆盖 cursor 时返回 `resync_required`，禁止静默拼接不完整输出；不根据文本内容猜测重复或增量。
...
shell, usually PowerShell or PowerShell 7 when available. Use Windows paths
and command syntax. Use terminal_write for input and terminal_signal with
...
描述的来源应该是 `ShellResolver` 的实际解析结果，而不是硬编码一个假设。建议 `shell="auto"` 作为默认值：Windows 优先可用的 PowerShell 7，否则 Windows PowerShell，同时支持显式 `cmd.exe`；macOS 优先用户 `$SHELL`，通常为 zsh；Linux 优先用户 `$SHELL`，不可用时回退 bash/sh；显式 shell 只能选择已解析且允许的 executable。
...
3. **session service**：继续复用 `TerminalSessionService`、已有 `terminal_sessions` 表、进程内 registry、ring buffer 和 Task 归属校验。不要新增表。
...
交互式 Tool renderer 继续由现有 `tool-part.tsx` 按 `data.kind` 和 `expand_layout` 路由，不按工具名增加散落的专用分支。
...
它们复用当前 `TerminalSession`、`TerminalWriteScheduler`、`TerminalOutputReconciler`、`TerminalViewport`、xterm.js 和 fit addon。

面板功能：shell、平台、cwd、运行状态、运行时长、exit code；ANSI/VT 原始输出由 xterm 解释；自动滚动；用户滚动离开底部时暂停跟随；新输出 unread 提示和“回到底部”；选择、复制、搜索；可选的输出下载；`resync_required` 时清空并按新 snapshot 重建；`read_only` 明确标识。
...
面板关闭只取消 WebSocket 订阅。Agent 仍可继续操作 session，用户重新打开时按照最后 cursor attach。
...
- [Tool UI](https://www.assistant-ui.com/docs/tools/tool-ui)：backend tool 可以只注册 UI renderer，renderer 应根据工具调用状态显示运行中、结果和错误；
...
2. 新增 Task 级 terminal store 和只读 preview connection；
3. 新增 TerminalPanel 和打开/聚焦事件；
4. 接入 attach、增量 output、status、exit、resync 和重连；
5. 在现有 `tool-part.tsx` 的 `kind + expand_layout` 路由下增加 `terminal-session` renderer；
...
- Windows、macOS、Linux 的 shell 描述与实际 resolver 结果一致；
- Windows 不出现可见控制台窗口；
