...
### 2.1 消费契约（不可破坏）
`workflow.py:137` 调 `build_chat_model(model_name, model_settings) -> BaseChatModel`，随后 `bind_tools(schemas, strict=True)` + `astream`。**返回对象仍是 `BaseChatModel` 即编排层零改动**。
...
| `define_agents.py` | 6 profile 迁移带前缀 model_name + 删 `base_url` |
...
- `define_agents.py`：6 profile（developer/developer_pro/reviewer/analyst/test/coder）全 `deepseek-v4-flash`/`deepseek-v4-pro`，硬编码 `base_url="https://api.deepseek.com"` + `api_key_env="DEEPSEEK_API_KEY"`。
...
### 4.4 profile 迁移
6 profile `model_name` 改 `deepseek/deepseek-v4-flash`/`deepseek/deepseek-v4-pro`；删 `base_url`（litellm 内置）；保留 `api_key_env="DEEPSEEK_API_KEY"`。`agent_profile.py` 默认值同步。
...
| 文件 | 问题 | 处理 |
|---|---|---|
| `test_langchain_bridge_assistant_content.py` | ① `_serialize_to_provider` 用 `langchain_openai._convert_message_to_dict`（删除后 import 失败）；② import `_ADDITIONAL_KWARGS_DROP_KEYS` 但当前 `langchain_bridge.py` 无此符号（既有不一致） | 序列化验证改 litellm 等价物或删私有序列化断言；R4 单独判定 |
...
| 风险 | 缓解 |
|---|---|
| R1：0.7.0 流式丢 `reasoning_content`（W1 不成立） | 冒烟先证实；若丢则 `ChatLiteLLMWithReasoning` 适配子类覆写 chunk 转换 |
...
- **Task 1（先行）**：改 `pyproject.toml` + `uv lock`；解压 wheel 核实 W1-W5；真实调 Key 冒烟 `ChatLiteLLM(deepseek/deepseek-v4-flash).astream`，打印 chunk 的 `additional_kwargs`/`usage_metadata`/`reasoning_content`，定论 reasoning_content/usage 透传事实。
- **Task 2**：`factory.py` 重写 + `model_settings.py` + `model_catalog.py` 归一化；删 `llm_provider/` 子包 + `model_http_pool.py` + `app/app.py` 清理。
- **Task 3**：profile 迁移（`define_agents.py` + `agent_profile.py`）。
...
- [ ] 真实调 Key 冒烟：reasoning_content / usage_metadata 透传事实明确
