 ...
 * (e.g. the git-hook `workspace_payload sync` running while the MCP server writes).
 */
...
export class DatabaseConnection {
   ...
   * — a git worktree removed and re-added, or `.workspace_payload/` deleted and
   ...
   * from what `configureConnection` requested. Surfaced in `workspace_payload status` so
   ...
   * The trigger is removing and recreating `.workspace_payload/` at the same path under
   * a long-lived process (`git worktree remove` + re-add, or `rm -rf
   * .workspace_payload` + `workspace_payload init`). Returns false when the inode is unchanged,
 ...
 */
export const DATABASE_FILENAME = 'workspace_payload.db';
