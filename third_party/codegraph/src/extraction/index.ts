...
function readGitignorePatterns(giPath: string): string {
  ...
  try {
    ignore().add(content).ignores('.workspace_payload-probe');
    return content;
  } catch {
  ...
  for (const line of content.split(/\r?\n/)) {
    try {
      ignore().add(line).ignores('.workspace_payload-probe');
      kept.push(line);
    } catch {
      dropped++;
    }
...
/**
 * Matcher for the project's `workspace_payload.json` `includeIgnored` patterns — the
...
/**
 * Matcher for the project's `workspace_payload.json` `exclude` patterns — paths to keep
...
/**
 * Matcher for the project's `workspace_payload.json` `include` patterns — first-party
...
/**
 * The included source files (`workspace_payload.json` `include`) for a scan root, or an
...
export class ScopeIgnore {
  ...
  constructor(
    ...
    /**
     * Project `workspace_payload.json` `exclude` patterns (#999), matched against the
    ...
    /**
     * Project `workspace_payload.json` `include` patterns — first-party source forced
 ...
 *      NOT opt that path in via `workspace_payload.json` `includeIgnored`. The gitignore
 ...
 * (#514) only for directories the project opted in via `workspace_payload.json`
 ...
 * opted into via `workspace_payload.json` `includeIgnored`. These are real repos the
 ...
 * `workspace_payload.json` `includeIgnored`; without that, a gitignored dir — including
 ...
 * directories the project opted in through `workspace_payload.json` `includeIgnored`
...
function collectGitFiles(repoDir: string, prefix: string, files: Set<string>, embeddedRoots?: Set<string>, includeIgnored: Ignore | null = null): void {
  ...
  // only when the project opted the directory in via `workspace_payload.json`
...
function getGitVisibleFiles(rootDir: string): Set<string> | null {
  try {
    ...
    // `workspace_payload.json` `include`. These are gitignored, so `git ls-files` never
 ...
 * directories opted in via `workspace_payload.json` `includeIgnored` (#622, #699) —
 ...
 * with it); a full `workspace_payload index` reconciles that.
 */
function getGitChangedFiles(rootDir: string): GitChanges | null {
  try {
    const changes: GitChanges = { modified: [], added: [], deleted: [] };
    // Custom extension → language overrides from the project's workspace_payload.json,
...
function collectGitStatus(repoDir: string, prefix: string, out: GitChanges, overrides?: Record<string, Language>, includeIgnored: Ignore | null = null, exclude: Ignore | null = null): void {
  ...
  // change — so `workspace_payload status` (which reads getChangedFiles) reports a
  ...
  for (const line of output.split('\n')) {
    ...
    // User `workspace_payload.json` `exclude` (#999) is project-root-relative, so it's
...
function scanDirectoryWalk(
  ...
  // Project `workspace_payload.json` `exclude` patterns (#999), rooted at the project so
  ...
  // Force-include first-party source whitelisted in `workspace_payload.json` `include`
...
export class ExtractionOrchestrator {
  ...
  async indexAll(
     ...
     * here on the main thread, where the workspace_payload.json overrides are loaded.
     */
  ...
  async indexFileWithContent(
    ...
    // Detect language (honoring the project's workspace_payload.json extension overrides)
