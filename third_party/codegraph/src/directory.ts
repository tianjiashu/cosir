 ...
 *
 * Manages the .workspace_payload/ directory structure for CodeGraph data.
 */
...
const DEFAULT_CODEGRAPH_DIR = '.workspace_payload';

let warnedBadDirName = false;

/**
 * Resolve the per-project data directory name, honoring the `CODEGRAPH_DIR`
 * environment override (default `.workspace_payload`). The override is a single path
 * segment that lives in the project root.
 *
 * Why this exists: two environments that share one working tree must NOT share
 * one `.workspace_payload/` — most concretely Windows-native and WSL (issue #636). The
 * daemon lockfile (`.workspace_payload/daemon.pid`) records a platform-specific pid and
 ...
 * daemons sharing one index risks corruption. Setting `CODEGRAPH_DIR=.workspace_payload-win`
 ...
 * default `.workspace_payload`, the active `CODEGRAPH_DIR` override, and any
 * `.workspace_payload-*` sibling. File-watching and the indexer skip ALL of these, so
...
/**
 * Get the .workspace_payload directory path for a project
 */
 ...
 * Requires both .workspace_payload/ directory AND workspace_payload.db to exist
 */
export function isInitialized(projectRoot: string): boolean {
  ...
  // Must have workspace_payload.db, not just .workspace_payload folder
  const dbPath = path.join(codegraphDir, 'workspace_payload.db');
  return fs.existsSync(dbPath);
}

/**
 * Find the nearest parent directory containing .workspace_payload/
 *
 * Walks up from the given path to find a CodeGraph-initialized project,
 * similar to how git finds .git/ directories.
 *
 * @param startPath - Directory to start searching from
 * @returns The project root containing .workspace_payload/, or null if not found
 */
/**
 ...
 * running the installer or `workspace_payload init` from `$HOME`, which auto-indexes the
 ...
 * (`packages/x/.workspace_payload/`), not at the workspace root the agent's cwd points
 ...
 * shaped by where the `.workspace_payload/` index(es) actually are:
...
/**
 * Contents of `.workspace_payload/.gitignore`. A single wildcard ignore keeps every
...
/**
 * Write `.workspace_payload/.gitignore` if it's absent, or upgrade a stale
...
/**
 * Create the .workspace_payload directory structure
 * Note: Only throws if workspace_payload.db already exists, not just if .workspace_payload/ exists.
 */
export function createDirectory(projectRoot: string): void {
  ...
  // .workspace_payload/ folder alone is fine
  ...
  // Write .gitignore inside .workspace_payload (create if absent, upgrade a stale
  // pre-wildcard default left by an older version — issue #788).
  ensureGitignore(path.join(codegraphDir, '.gitignore'));
}

/**
 * Remove the .workspace_payload directory
 */
export function removeDirectory(projectRoot: string): void {
  ...
  // Verify .workspace_payload is a real directory, not a symlink pointing elsewhere
...
/**
 * Get all files in the .workspace_payload directory
 */
export function listDirectoryContents(projectRoot: string): string[] {
  ...
  function walkDir(dir: string, prefix: string = ''): void {
    ...
    for (const entry of entries) {
      ...
      // Skip symlinks to prevent following links outside .workspace_payload
...
/**
 * Get the total size of the .workspace_payload directory in bytes
 */
...
/**
 * Ensure a subdirectory exists within .workspace_payload
 */
...
/**
 * Check if the .workspace_payload directory has valid structure
 */
export function validateDirectory(projectRoot: string): {
  ...
  if (!fs.statSync(codegraphDir).isDirectory()) {
    errors.push('.workspace_payload exists but is not a directory');
    return { valid: false, errors };
  }
  ...
  if (!ensureGitignore(gitignorePath) && !existedBefore) {
    ...
    errors.push('.gitignore missing in .workspace_payload directory and could not be created');
  }
