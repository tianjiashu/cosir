...
const EXHAUSTION_REASON =
  'OS watch/file limit exhausted; auto-sync disabled. Run `workspace_payload sync` ' +
...
const INOTIFY_LIMIT_REASON =
  ...
  'restart, or run `workspace_payload sync` (or install git sync hooks) to refresh.';
 ...
 * log once — partial live-watch (with `workspace_payload sync` as the backstop) is far
 ...
 * - Ignores .workspace_payload/ and .git/ regardless of .gitignore
...
export class FileWatcher {
  ...
  start(): boolean {
    ...
    // there; callers fall back to manual `workspace_payload sync` or git sync hooks.
  ...
  private isAlwaysIgnored(rel: string): boolean {
    ...
    // sibling like `.workspace_payload-win` a second environment (Windows/WSL) created
   ...
   * installed keep firing, and `workspace_payload sync` covers the unwatched remainder.
        ...
        if (this.lockRetryCount > MAX_LOCK_RETRIES) {
          this.degrade(
            'CodeGraph file lock held by another process past the retry budget; ' +
              'auto-sync disabled. Run `workspace_payload sync` once the other writer finishes ' +
        ...
        if (this.syncFailureRetryCount > MAX_SYNC_FAILURE_RETRIES) {
          this.degrade(
            `CodeGraph auto-sync failed ${this.syncFailureRetryCount} times in a row; ` +
              'auto-sync disabled. Run `workspace_payload sync` (or install git sync hooks) to ' +
