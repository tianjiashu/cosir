/**
 * Tests for the CI/scripting fields `workspace_payload status --json` exposes (issue
...
describe('workspace_payload status --json — CI fields (#329)', () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-status-json-'));
  });
  afterEach(() => {
  ...
  it('status --json on an UNINITIALIZED project reports version + indexPath + lastIndexed:null', () => {
    ...
    expect(out.indexPath as string).toContain('.workspace_payload');
...
describe('index completeness marker (index_state)', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-index-state-'));
  });
  ...
  it('a run killed mid-index leaves state=indexing, and status --json surfaces it', async () => {
    ...
    const db = new DatabaseSync(path.join(tempDir, '.workspace_payload', 'workspace_payload.db'));
    db.prepare(
