...
describe('FileWatcher', () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-watcher-'));
  ...
  describe('watch-resource exhaustion (#876)', () => {
    ...
    const mkEmptyDir = () => fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-exhaust-'));
    ...
    it('warns once (NOT degrade) when Linux inotify watches are exhausted (ENOSPC)', () => {
      ...
      try {
        ...
        const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-inotify-'));
  ...
  describe('filtering', () => {
    ...
    it('should ignore .workspace_payload directory changes', async () => {
      const syncFn = vi.fn().mockResolvedValue({ filesChanged: 0, durationMs: 0 });
      const watcher = newWatcher(syncFn, { debounceMs: 200 });
      ...
      // A .workspace_payload event — FileWatcher's `isAlwaysIgnored` filter must drop
      // it before scheduling sync.
      __emitWatchEventForTests(testDir, '.workspace_payload/db.sqlite');
