 ...
 *
 * A CodeGraph index is resolved by walking up to the nearest `.workspace_payload/`.
...
describe('detectWorktreeIndexMismatch (issue #155)', () => {
  let mainRepo: string;   // main checkout — owns the .workspace_payload index
  ...
  it('warning names both trees and the fix', () => {
    ...
    expect(msg).toContain('workspace_payload init');
  });
});
...
describe('worktree mismatch surfaces on hot read tools (issue #155)', () => {
  ...
  it('prefixes a compact notice on codegraph_search run from a nested worktree', async () => {
    ...
    expect(text).toContain('workspace_payload init');
  });
 ...
 * checkout's index" — true while the worktree had no `.workspace_payload/` of its own —
 ...
 * "run workspace_payload init -i" advice would fragment the unified index. (#1031, #1033)
 */
describe('detectWorktreeIndexMismatch — nested repos covered by the parent index (#1031, #1033)', () => {
  let parent: string;     // super-repo that owns the .workspace_payload index
