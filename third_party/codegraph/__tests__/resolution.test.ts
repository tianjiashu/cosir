...
describe('Resolution Module', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-resolution-test-'));
  });
...
#include "utils/helpers.hpp"`;
    ...
    it('should discover include directories from compile_commands.json', () => {
      ...
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-cpp-test-'));
      try {
        const compileDb = [
          {
    ...
    it('connects #include to the real header file via include-dir scan (end-to-end)', async () => {
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-cpp-e2e-'));
      try {
        fs.mkdirSync(path.join(tempProject, 'include'), { recursive: true });
        fs.mkdirSync(path.join(tempProject, 'src'), { recursive: true });
        fs.writeFileSync(
        ...
        const db = DatabaseConnection.open(path.join(tempProject, '.workspace_payload', 'workspace_payload.db'));
...
`
      ...
      cg = await CodeGraph.init(tempDir, { index: true });
      const db = DatabaseConnection.open(path.join(tempDir, '.workspace_payload', 'workspace_payload.db'));
      const edges = db
        .getDb()
        .prepare(
  ...
  describe('PHP Include Resolution', () => {
    ...
    it('resolves require_once to a file→file imports edge (#660)', async () => {
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-php-e2e-'));
      try {
        fs.mkdirSync(path.join(tempProject, 'src'), { recursive: true });
        fs.writeFileSync(
    ...
    it('resolves a subdirectory include path to the correct file (#660)', async () => {
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-php-subdir-'));
      try {
    ...
    it('does not mis-connect an unresolvable include to a same-named file elsewhere (#660)', async () => {
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-php-misresolve-'));
      try {
