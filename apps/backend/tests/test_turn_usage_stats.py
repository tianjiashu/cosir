"""``ConversationRunUsageStats`` 单元测试。
...
def test_add_usage_metadata_none_output_details_is_safe() -> None:
    """嵌套 output_token_details 为 None 时不应抛 AttributeError，reasoning 计为零。
...
def test_add_message_usage_alias_matches_add_usage_metadata() -> None:
    ...
    delegated = ConversationRunUsageStats()
    direct = ConversationRunUsageStats()
