...
class LogStoreTests(unittest.TestCase):
    ...
    def test_initialize_creates_schema_indexes_and_user_version(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("idx_log_entries_task_ts", indexes)
            self.assertIn("idx_log_entries_run_ts", indexes)
    ...
    def test_insert_and_query_by_trace_returns_ascending_entries(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual([entry.event_name for entry in entries], ["first", "second"])
    ...
    def test_recent_query_returns_descending_limited_entries(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual([entry.event_name for entry in entries], ["third", "second"])

    def test_level_and_attributes_round_trip(self) -> None:
        """校验 level 过滤和 attributes JSON 读写。
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(entries[0].attributes["tool_name"], "read_file")

    def test_time_range_task_run_event_and_invalid_limit(self) -> None:
        """校验时间范围、task/run/event 查询和非法 limit。
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual([entry.event_name for entry in entries], ["target_event"])
            with self.assertRaises(ValueError):
                store.query(LogQuery(limit=0))
            with self.assertRaises(ValueError):
                store.query(LogQuery(order="sideways"))

    def test_non_object_attributes_json_returns_empty_attributes(self) -> None:
        """校验非对象 attributes_json 会降级为空字典。
        ...
        异常:
            AssertionError: 如果非对象 attributes 没有被隔离。
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with sqlite3.connect(database) as connection:
                connection.execute(
                    ...
                    INSERT INTO log_entries(
                        ts, level, logger_name, event_name, message,
                        attributes_json, truncated
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
            ...
            entries = store.query(LogQuery(event_name="bad_attrs", limit=10))
            self.assertEqual(entries[0].attributes, {})
