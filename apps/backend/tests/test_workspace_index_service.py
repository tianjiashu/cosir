...
class _FakeLifecycleWithClient:
    ...
    def __init__(self, client, readiness: WorkspaceIndexReadiness | None = None) -> None:
        self._client = client
...
def test_prepare_health_check_none_client_degrades():
    ...
    class _NoClientLifecycle:
        ...
        def ensure_ready(self, _wp: str) -> WorkspaceIndexReadiness:
            raise AssertionError("ensure_ready 不应被调用")

    svc = WorkspaceIndexService(_NoClientLifecycle(), bus)  # type: ignore[arg-type]
    result = svc.prepare("ws-1", "/ws/root")
