...
并发额度的风险。新实现用单条 ``UPDATE ... WHERE status IN (active) [AND parent_run_id=?]
RETURNING id`` 原子置 failed。

本测试以 MagicMock 桩替代 session，验证：
- 生成的 UPDATE 语句必须带 ``status IN (active)`` 条件（保证只动活跃记录）；
- 单 turn 恢复带 ``parent_run_id`` 过滤，全局恢复不带该过滤；
...
def test_scoped_recovery_targets_active_and_parent_turn() -> None:
    """单 turn 恢复：UPDATE 必须带 ``status IN (active)`` 与 ``parent_run_id`` 过滤。"""

    crud, session = _make_crud_with_session(returning_ids=[7, 8])

    affected = crud.fail_active_delegations(error="recovery", parent_run_id=1)
...
def test_global_recovery_has_no_parent_turn_filter() -> None:
    """全局恢复（parent_run_id=None）：UPDATE 不带 parent_run_id 过滤，仅按活跃状态。"""
    ...
    text = _compiled_text(session)
    assert "status" in text
    assert "parent_run_id" not in text
...
def test_no_active_records_returns_empty() -> None:
    ...
    affected = crud.fail_active_delegations(error="recovery", parent_run_id=99)
