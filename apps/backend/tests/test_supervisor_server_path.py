...
``third_party/workspace_payload/dist/agent-kernel/server.js``（导致 Kernel 启动报
...
def test_server_script_path_points_to_codegraph_vendor():
    """路径应指向 codegraph vendor，而非已废弃的 workspace_payload 目录。"""
    ...
    # 关键断言：修正 bug——根目录必须是 third_party/codegraph，不是 third_party/workspace_payload。
