...
class CodeGraphLifecycleService:
    ...
    def get_client(self) -> CodeGraphKernelClient | None:
        """返回底层 Kernel RPC 客户端（供调用方做健康快检）。

        参数:
            无。

        返回:
            CodeGraphKernelClient | None: 客户端实例；若未注入则返回 None。

        异常:
            无。

        副作用:
            无。返回的实例可能处于「进程已退出」状态，调用方需以 `ping` 等探测判定
            真实可达性，本方法不保证进程存活。
        """
