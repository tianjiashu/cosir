...
单一职责：给定一个 ``ConversationRunRecord``，解析出它所属 task 的 workspace 根路径
...
- 不负责：索引准备（归 CodeGraphLifecycleService / CodeGraphIndexPrepareHook）、
  turn 状态管理（归 ConversationRunStateService）、事件发布（归 workspace 创建即索引链路）。
"""
...
class ConversationRunWorkspaceResolver:
    ...
    def resolve(self, turn: ConversationRunRecord) -> str | None:
