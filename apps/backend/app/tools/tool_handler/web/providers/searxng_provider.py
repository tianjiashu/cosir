"""SearXNG API provider."""

import os

import httpx

from app.config.settings import Settings
from app.tools.tool_handler.web.web_provider import (
    WebExtractItem,
    WebProviderUnavailableError,
    WebSearchItem,
)


class SearxngProvider:
    """通过自托管 SearXNG 实例提供网页搜索。"""

    name = "searxng"

    def __init__(self, base_url: str | None = None) -> None:
        """初始化 SearXNG Provider。

        参数:
            base_url: 可选的 SearXNG 根地址；省略时从环境变量读取。

        返回:
            无。

        异常:
            无。

        副作用:
            读取 SEARXNG_URL 环境变量。
        """

        self._base_url = base_url if base_url is not None else os.environ.get("SEARXNG_URL", "")

    @property
    def display_name(self) -> str:
        """返回 SearXNG 的可读名称。

        参数:
            无。

        返回:
            Provider 的可读名称。

        异常:
            无。

        副作用:
            无。
        """

        return "SearXNG"

    def is_available(self) -> bool:
        """判断 SearXNG 根地址是否存在。

        参数:
            无。

        返回:
            已配置根地址时返回 True。

        异常:
            无。

        副作用:
            无。
        """

        return bool(self._base_url)

    def supports_search(self) -> bool:
        """返回 SearXNG 的搜索能力状态。

        参数:
            无。

        返回:
            始终返回 True。

        异常:
            无。

        副作用:
            无。
        """

        return True

    def supports_extract(self) -> bool:
        """返回 SearXNG 的正文提取能力状态。

        参数:
            无。

        返回:
            始终返回 False。

        异常:
            无。

        副作用:
            无。
        """

        return False

    def supported_extract_formats(self) -> frozenset[str]:
        """返回 SearXNG 支持的正文格式集合。

        参数:
            无。

        返回:
            空集合，因为 SearXNG 不支持正文提取。

        异常:
            无。

        副作用:
            无。
        """

        return frozenset()

    def missing_configuration_message(self) -> str:
        """返回 SearXNG 未配置时的英文诊断信息。

        参数:
            无。

        返回:
            简洁英文配置说明。

        异常:
            无。

        副作用:
            无。
        """

        return "SearXNG is unavailable: set SEARXNG_URL."

    def search(self, query: str, limit: int) -> list[WebSearchItem]:
        """使用 SearXNG JSON API 执行网页搜索。

        参数:
            query: 搜索关键词。
            limit: 最多返回的搜索结果数量。

        返回:
            归一化后的网页搜索结果。

        异常:
            WebProviderUnavailableError: 未配置 SearXNG 根地址时抛出。
            httpx.HTTPError: SearXNG 请求失败时抛出。

        副作用:
            发起 SearXNG 网络请求。
        """

        if not self.is_available():
            raise WebProviderUnavailableError(self.missing_configuration_message())
        with httpx.Client(timeout=Settings.WEB_REQUEST_TIMEOUT_SECONDS) as client:
            response = client.get(
                f"{self._base_url.rstrip('/')}/search",
                params={"q": query, "format": "json"},
            )
            response.raise_for_status()
        payload: object = response.json()
        raw_results = payload.get("results") if isinstance(payload, dict) else None
        if not isinstance(raw_results, list):
            return []
        return [
            WebSearchItem(
                title=str(item.get("title", "")),
                url=str(item.get("url", "")),
                description=str(item.get("content", "")),
                position=position,
            )
            for position, item in enumerate(raw_results[:limit], start=1)
            if isinstance(item, dict) and item.get("url")
        ]

    def extract(self, urls: list[str], output_format: str, char_limit: int) -> list[WebExtractItem]:
        """拒绝 SearXNG 不支持的正文提取调用。

        参数:
            urls: 待提取的网页地址列表。
            output_format: 调用方请求的网页正文格式。
            char_limit: 单页正文最大字符数。

        返回:
            不返回结果。

        异常:
            WebProviderUnavailableError: 始终抛出，说明 SearXNG 不支持正文提取。

        副作用:
            无。
        """

        raise WebProviderUnavailableError("SearXNG does not support web extraction.")
