...
class InvalidToolOutcome(str, Enum):
    ...
    - ``REPAIR``：非法调用命中已注册工具名（视为真实调用意图，仅字段非法），需把
      ...
      情形 b 无合法工具时由 model_node 直接回写 RuntimeContext 并回流 model 节点）。
...
class ModelToolHelper:
    ...
    def build_invalid_tool_call_repair_message(
        ...
        ``SystemMessage`` 写进 ``RuntimeContext``。结构：
        ...
        budget = ModelToolHelper.INVALID_TOOL_CALL_TOTAL_BUDGET_CHARS
        truncated = False

        for repair_data in repair_datas[: ModelToolHelper.INVALID_TOOL_CALL_SUMMARY_LIMIT]:
            ...
            if len(redacted_args) > ModelToolHelper.INVALID_TOOL_ARGS_PREVIEW_CHARS:
                redacted_args = (
                    redacted_args[: ModelToolHelper.INVALID_TOOL_ARGS_PREVIEW_CHARS]
                    + "...[truncated]"
                )
        ...
        if truncated:
            ...
            # 末尾说明文本同样计入整体预算：若 body + 说明超出预算，对 body 做最终裁剪，
            # 保证 header + body + 说明整体不超过 INVALID_TOOL_CALL_TOTAL_BUDGET_CHARS。
            projected = len(body) + len(truncation_note)
            if projected > budget:
                overflow = projected - budget
                body = (
    ...
    def decide_invalid_tool_handling(
        ...
        for invalid_tool_call in invalid_tool_calls:
            mentions_tool = ModelToolHelper.invalid_tool_call_mention_tool_name(
                invalid_tool_call,
