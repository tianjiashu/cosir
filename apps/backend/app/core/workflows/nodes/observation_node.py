...
1. **协作取消统一收口**：无论取消产生自 ``model``（请求前 / 流式中）还是 ``tools``
   （执行前），都在此一次性收口——先分发本批已产生的工具结果（终态事件 + 模型上下文），
   再统一 ``ToolCallLifecycleManager.cancel`` 收口全部挂起调用并落定取消终态；随后由图
   路由到 ``pause``（图不结束），该 run 仍可续跑；
...
async def _observe_node(state: ReactGraphState) -> dict:
    ...
    2. **协作取消统一收口**：分发完成后若取消已产生（``cancel_requested`` 为真或 run 已
       取消），统一收口本 manager 全部挂起调用并落定 cancelled 终态，不再进错误上限判定
       与后续推理；图经 ``_after_observe`` 停在 ``pause``（不结束），该 run 仍可续跑。
    ...
    返回:
        需要合并回 graph state 的增量（各分支均回写 ``tool_call_lifecycle``）：协作取消
        分支返回 ``{"tool_error_count", "cancel_requested": True, "tool_call_lifecycle"}``
        （由图经 ``_after_observe`` 路由到 ``pause``）；空结果且无修复提示返回
        ``{"tool_call_lifecycle"}``；仅修复提示返回 ``{"tool_error_count",
        "tool_call_lifecycle"}``；错误上限分支（含失败落定竞态落空）返回
        ``{"tool_error_count", "terminal": True, "tool_call_lifecycle"}``；正常分支返回
        ``{"tool_error_count", "tool_call_lifecycle"}``，供 ``_after_observe`` 路由回
        ``model``。

    副作用:
        - 分发阶段经 stream writer 发出 ``ToolCallStatusChangedEvent``，并把
          ``ToolMessage`` 写回 ``RuntimeContextManager``；
        - **协作取消分支是取消的唯一收口点**：经 ``ToolCallLifecycleManager.cancel`` 统一
          收口全部挂起调用（不留「一半一半」），并经 ``RuntimeOperations.cancel_run_if_running``
          落定取消终态；
    ...
    # tools 节点产出的结果摘要（取消分支可能来自尚未执行过工具步的节点，彼时由取消方显式
    # 置空；这里仍用 ``get`` 防御，避免任何路径漏置键时整条收口链路崩溃）。
