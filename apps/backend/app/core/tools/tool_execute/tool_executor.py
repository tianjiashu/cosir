...
class ToolExecutor:
    ...
    def _execute_inner(
        ...
        副作用:
            ...
            （``thread`` 在当前线程直跑 / ``process`` 子进程隔离 + 硬超时强杀）；
            可能因权限或参数校验失败而短路返回，不进入执行阶段。本方法不动
            Transport snapshot；终态投影由 :meth:`execute` 的单一出口负责。
        """
        ...
        # 阶段二：文件状态协调的准备阶段。准备、预算与 early-return 逻辑收口在
