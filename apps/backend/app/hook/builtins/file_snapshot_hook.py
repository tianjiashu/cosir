...
单一职责：在文件类工具成功执行后，从工具观察的 ``artifact_data["changes"]`` 采集反向
...
class FileSnapshotHook(HookBase):
    ...
    可落库快照（仅成功、带 run_id / task_id、且 observation.artifact_data 含 changes 的文件工具）。
    """
    ...
    def execute(self, context: HookContext) -> HookResult:
        ...
        放弃采集，避免快照落入空串归属的 seq 命名空间）；无 ``observation.artifact_data``
        ...
        if not context.run_id or not context.task_id:
            return HookResult.allow()
        data = observation.artifact_data
        if not data:
            return HookResult.allow()
    ...
    def _record(
        ...
        """
        data = observation.artifact_data or {}
        changes = data.get("changes")
        if not isinstance(changes, list):
            log.warning(
                "file_snapshot_changes_invalid",
                extra={
                    "msg": "工具观察 artifact_data.changes 非 list，跳过文件快照采集",
...
def _change_diff_stats(changes: list[dict]) -> list[tuple[int, int]]:
    ...
    参数:
        changes: ``artifact_data["changes"]`` 中的单文件变更字典列表，每个含
