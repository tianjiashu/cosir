...
class HookContext:
    ...
    def from_locatable(
        ...
        ``workspace_id`` / ``task_id`` / ``turn_id`` 定位字段，但字段宿主不同。
        ...
        参数:
            ...
            turn: 可选轮次记录，提供 ``turn_id``（运行期事件传 ``turn``，
                ``ToolExecutionContext`` 自身已带 ``turn_id`` 时不必再传）。
        ...
        return cls(
            ...
            turn_id=turn_id or getattr(locatable, "turn_id", None),
        )
