...
- **自定义工具系统（不基于 LangGraph）**：`ToolDefinition` 是工具契约单一事实来源；内置文件工具包含 `read_file` / `write_file` / `patch` / `search_content` / `find_files` / `list_directory` / `delete`，全部继承 `tool_handler/tool_base.HandlerBase`，由 `core` 调度执行，采用分级隔离（thread 直跑 / process 子进程+硬超时强杀）。
...
后端日志落盘位置（单个文件上限 5MB，按本地日期切换；同日大小分片追加 `.1.log`、`.2.log` 等序号）：

- 应用结构化日志：由 `Settings.LOG_DIR` 配置为 `backend-YYYY-MM-DD.log`
- 桌面开发时的 uvicorn stdout/stderr：由 Tauri supervisor 写入应用 runtime 目录的 `backend-console-YYYY-MM-DD.log`
...
桌面窗口关闭或按 `Ctrl+C` 会终止后端及其子进程。开发期后端使用应用 runtime 目录下的 `uv-cache`，不依赖用户全局 uv 缓存权限；stdout/stderr 会写入桌面运行目录的 `backend-console-YYYY-MM-DD.log`。
