...
export const Thread: FC<ThreadProps> = ({
  ...
  return (
    <ThreadComponentsContext.Provider value={components}>
      <ThreadRoot isEmpty={isEmpty} autoFocus={autoFocus} taskId={taskId} />
...
const Composer: FC<{ autoFocus: boolean; taskId?: number }> = ({
  autoFocus,
  taskId,
}) => {
  return (
    <ComposerPrimitive.Root className="aui-composer-root relative flex w-full flex-col">
      <ComposerPrimitive.AttachmentDropzone render={<ComposerSurface data-slot="aui_composer-shell" />}><ComposerAttachments /><ComposerPrimitive.Input asChild>
        <ComposerInput
        ...
        />
      </ComposerPrimitive.Input><ComposerAction taskId={taskId} /></ComposerPrimitive.AttachmentDropzone>
    </ComposerPrimitive.Root>
  );
};

const ComposerAction: FC<{ taskId?: number }> = ({ taskId }) => {
  return (
    <div className="aui-composer-action-wrapper relative flex items-center justify-between">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <AuiIf condition={(s) => s.thread.isRunning}>
          {/* render 经 asChild 合并：内部默认 onClick（abort 连接）与本 StopButton onClick
              同时执行——先断前端连接即时响应，再发真实 cancel 落定后端 cancelled 状态。
              StopButton 内部通过官方 useAssistantTransportState 读取 turnId，无需此处透传。 */}
          <ComposerPrimitive.Cancel
            render={
              <StopButton taskId={taskId ?? null} />
            }
          />
        </AuiIf>
      </div>
