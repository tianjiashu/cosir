...
function ToolFallbackResult({
  ...
  if (
    ...
    (typeof result === "object" &&
      result !== null &&
      "kind" in result &&
      (result.kind === "tool-error" || result.kind === "tool-cancelled"))
  )
    return null;
...
function ToolFallbackError({
  ...
  const isCancelled =
    (status?.type === "incomplete" && status.reason === "cancelled") ||
    envelope?.kind === "tool-cancelled";
  const isFailed =
    isError ||
    envelope?.kind === "tool-error" ||
    (status?.type === "incomplete" && status.reason === "error");
  if (!isCancelled && !isFailed) return null;

  const error =
    envelope?.kind === "tool-error" && "message" in envelope
      ? envelope.message
...
const ToolFallbackImpl: ToolCallMessagePartComponent = ({
  ...
  return (
    <ToolFallbackRoot open={open} onOpenChange={setOpen}>
      ...
      />
      <ToolFallbackContent>
        <ToolFallbackError status={status} result={result} isError={isError} />
        <ToolFallbackArgs
          argsText={argsText}
