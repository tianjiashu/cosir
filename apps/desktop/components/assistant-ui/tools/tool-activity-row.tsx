...
export function ToolActivityRow({ icon, title, meta, status, onOpen, openLabel, disabled = false, className }: ToolActivityRowProps) {
  const canOpen = onOpen !== null && onOpen !== undefined && !disabled;
  const open = () => { if (canOpen) onOpen(); };
  return (
    <button
      type="button"
      className={cn(DISCLOSURE_ROW_CLASS, "group rounded-md px-2 text-left hover:bg-muted/60 disabled:cursor-default", className)}
      disabled={disabled}
      onClick={open}
