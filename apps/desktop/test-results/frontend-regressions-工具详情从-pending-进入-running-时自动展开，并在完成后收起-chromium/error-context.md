...
- Name: frontend-regressions.spec.ts >> 工具详情从 pending 进入 running 时自动展开，并在完成后收起
- Location: tests\e2e\frontend-regressions.spec.ts:80:5
...
Locator: locator('[data-role="assistant"]').last().getByText('未找到匹配', { exact: true })
Expected: visible
Timeout: 2000ms
...
Call log:
  ...
  - waiting for locator('[data-role="assistant"]').last().getByText('未找到匹配', { exact: true })
...
- complementary:
  ...
  - navigation "工作区任务列表":
    ...
    - button "demo 1"
    - button "更多 demo 操作"
    - button "工具生命周期回归 1/1/2026"
    - button "更多 工具生命周期回归 操作"
- main:
  - paragraph: 对话
  - paragraph: 已存在任务
  - text: demo
