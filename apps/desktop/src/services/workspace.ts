 ...
 * 在捕获模式下执行目录选择创建工作区。
 *
 * 用于删除工作区后的引导：自动触发目录选择器，失败仅记录日志，
 * 不向上抛出以免中断删除流程的后续状态收口。
 *
 * @returns 创建成功返回 true；用户取消或失败返回 false。
 *
 * @sideeffect 成功时写入 workspaceStore；失败时写入日志。
 */
export async function pickAndCreateWorkspaceSafe(): Promise<boolean> {
  try {
    const workspace = await pickAndCreateWorkspace();
    return workspace !== null;
  } catch (err) {
    logError("删除工作区后引导新建失败", err, { module: "workspaceService" });
    return false;
  }
}
