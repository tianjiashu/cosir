...
export class MCPEngine {
  ...
  // (or null forever if no .workspace_payload/ ever turned up — that's a valid
  ...
  /**
   * Walk up from `searchFrom` to find the nearest `.workspace_payload/` and open it.
  ...
  private async doInitialize(searchFrom: string): Promise<void> {
    ...
    const resolvedRoot = findNearestCodeGraphRoot(searchFrom);
    if (!resolvedRoot) {
      // No .workspace_payload/ above searchFrom. Sessions may still discover one later via roots/list
  ...
  private startWatching(): void {
    ...
    const started = this.cg.watch({
      ...
      onDegraded: (reason) => {
        ...
        // (`workspace_payload sync` / git sync hooks).
    ...
    if (started) {
      ...
      process.stderr.write(
        '[CodeGraph MCP] File watcher unavailable on this platform — run `workspace_payload sync` to refresh the graph after changes.\n'
      );
    }
  }
