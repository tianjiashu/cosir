 ...
 * `workspace_payload.json`); when present its extensions count as indexable in addition
 ...
 *
 * `overrides` is the project's validated custom extension → language map (from
 * `workspace_payload.json`); when present its mappings take precedence over the built-in
