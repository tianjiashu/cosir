...
export default defineConfig({
  test: {
     ...
     * mcp-roots) spawn `dist/bin/workspace_payload.js serve --mcp` with `process.execPath`
     * and rely on the child inheriting `process.env`. On a Node >= 25 dev machine
     * the CLI's hard-block (src/bin/workspace_payload.ts) would otherwise exit the child
     ...
     * users (it's still enforced when `workspace_payload` is invoked directly) while
    ...
    env: {
       ...
       * telemetry state into the contributor's real ~/.workspace_payload and count test
