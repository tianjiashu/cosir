...
// A temp dir standing in for the installed @colbymchenry/workspace_payload main package.
...
function writeLauncher(binDir: string): void {
  ...
  const p = path.join(binDir, 'workspace_payload');
...
describe.skipIf(!CAN_NET)('npm-shim download fallback (local HTTPS)', () => {
  ...
  beforeAll(async () => {
    ...
    // Build a fake bundle archive (workspace_payload-<target>/bin/workspace_payload), like a real release asset.
  ...
  it('downloads, verifies the checksum, extracts, and execs the bundle', async () => {
    ...
    expect(fs.existsSync(path.join(cache, 'bundles', `${target}-5.0.0-net`, 'bin', 'workspace_payload'))).toBe(true);
  }, 20000);
  ...
  it('prunes older cached bundles after downloading a new one (#1074)', async () => {
    ...
    expect(fs.existsSync(path.join(bundles, `${target}-6.0.0-new`, 'bin', 'workspace_payload'))).toBe(true);
