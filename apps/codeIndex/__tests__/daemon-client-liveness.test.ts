...
describe('Daemon.reapDeadClients', () => {
  ...
  const makeDaemon = () => new Daemon('/tmp/workspace_payload-reap-unit-test', { idleTimeoutMs: 0 }) as any;
...
describe('Daemon.backstopShouldExit', () => {
  ...
  const makeDaemon = () => {
    const d = new Daemon('/tmp/workspace_payload-backstop-unit-test', { idleTimeoutMs: 0, maxIdleMs: 1000 }) as any;
