 ...
 * failed with a misleading "CodeGraph not initialized. Run 'workspace_payload init'."
 *
...
describe('MCP project resolution via roots/list (issue #196)', () => {
  let cwdDir: string;     // where the server is launched — has NO .workspace_payload
  let projectDir: string; // the real indexed project the client reports
  let child: ChildProcessWithoutNullStreams | null = null;

  beforeEach(() => {
    cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-mcp-cwd-'));
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-mcp-proj-'));
  });
