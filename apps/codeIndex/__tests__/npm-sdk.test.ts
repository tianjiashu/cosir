 ...
 * `require("@colbymchenry/workspace_payload")` on their OWN Node by re-exporting the
 * compiled library that ships inside the per-platform optionalDependency
 * (@colbymchenry/workspace_payload-<target>/lib/dist/index.js).
 *
...
// SDK's `require.resolve('@colbymchenry/workspace_payload-<target>/...')` walks to it.
function makeConsumer(): { root: string; mainPkg: string } {
  const root = mkTmp('consumer');
  const mainPkg = path.join(root, 'node_modules', '@colbymchenry', 'workspace_payload');
