 ...
 * (`Fatal process out of memory: Zone`) that crashed `workspace_payload index` on large
...
describe('buildRelaunchArgv', () => {
  it('places the wasm flags first, then the script and its args', () => {
    expect(buildRelaunchArgv('/x/workspace_payload.js', ['index', '/repo'], [])).toEqual([
      '--liftoff-only',
      '/x/workspace_payload.js',
      'index',
      '/repo',
    ]);
  });
  ...
  it('preserves other existing node flags without duplicating ours', () => {
    expect(
      buildRelaunchArgv('/x/workspace_payload.js', ['status'], ['--liftoff-only', '--enable-source-maps'])
    ).toEqual(['--liftoff-only', '--enable-source-maps', '/x/workspace_payload.js', 'status']);
  });
