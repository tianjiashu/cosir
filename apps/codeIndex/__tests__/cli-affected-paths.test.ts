/**
 * `workspace_payload affected` input-path normalization (#825).
 *
...
describe('workspace_payload affected — input path normalization (#825)', () => {
  let tempDir: string;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace_payload-affected-paths-'));
