...
describe('Language Detection', () => {
  ...
  it('should detect Nix files', () => {
    ...
    expect(detectLanguage('pkgs/development/tools/misc/workspace_payload/default.nix')).toBe('nix');
...
describe('Nested gitlink repos (#1031, #1033)', () => {
  ...
  // re-include only via `workspace_payload.json` `includeIgnored`.
  ...
  it('re-includes a gitignored gitlink when workspace_payload.json includeIgnored opts in (#1065)', async () => {
    const { execFileSync } = await import('child_process');
    const git = (cwd: string, ...args: string[]) => execFileSync('git', args, { cwd, stdio: 'pipe' });
    ...
    fs.writeFileSync(path.join(root, '.gitignore'), 'benchmark/repos/\n');
    fs.writeFileSync(path.join(root, 'workspace_payload.json'), JSON.stringify({ includeIgnored: ['benchmark/repos/'] }));
    git(root, 'add', '.gitignore', 'workspace_payload.json');
